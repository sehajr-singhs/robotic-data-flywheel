...
\emph{\numIter} flywheel iterations. Curation changes the \emph{cost}
curve: curated relabeling---scoring failures by how close they came before
deciding to label---reaches \curatedFinal\ with \curatedQueries\ oracle
queries, while blind relabeling needs \blindQueries\ to reach
\relabelFinal: curation buys $\sim$1.6$\times$ the performance per query. Both
rules tolerate labeling noise, and curation degrades less under it
(\curatedDrop\% vs.\ \blindDrop\% loss at high noise). The effect survives
on a harder task, where relabeling still compounds while no feedback stays
flat. We release the framework, a deterministic multi-seed study, and a
...
60 deployment rollouts collected per iteration. The relabeling oracle is a
...
longer pushes, horizon $80$). The full study runs in under an hour on one
CPU core with numpy only.
...
for 72\% of the performance. At a matched budget of $\sim$7,500 queries,
curated relabeling ($\curatedFinal$) outperforms blind relabeling
($\sim$0.39). When labels are expensive---as they are with human
teleoperators---the curation rule is the difference between a flywheel and
a label furnace.

\textbf{Result 5: curation dampens oracle labeling noise.}
...
\textbf{Result 6: the flywheel effect survives task difficulty.}
...
\emph{cost}---less than half the queries for 72\% of the performance---and
Result 5 shows it also wins on \emph{robustness} to labeling mistakes
(\curatedDrop\% vs.\ \blindDrop\% loss). Blind ingestion is the right
rule only when labels are free and perfect; curation is the right rule in
the cost regime industrial cells actually face.
