...
from .curation.strategies import STRATEGIES, STRATEGY_MAKERS
...
from .policies.mlp import MLPPolicy, collect_trajectories, train_bc
...
def _bc_loss(policy, dataset: list, img_size: int = IMG_SIZE) -> float:
    """BC MSE on the training set — the memorization/overfitting proxy.

    When blind relabeling floods the dataset, the policy's training loss
    keeps dropping while held-out success collapses; the train-vs-eval gap
    is the mechanism's diagnostic.
    """
    import numpy as np

    states = np.concatenate([t.states for t in dataset], axis=0)
    acts = np.concatenate([t.actions for t in dataset], axis=0)
    if isinstance(policy, MLPPolicy):
        return float(policy._mse(states.astype(np.float64), acts.astype(np.float64)))
    # torch CNN: reuse the policy's image tensor conversion
    import torch
    from torch import nn

    imgs = np.concatenate([t.images for t in dataset], axis=0)
    X = policy._tensor(imgs)
    y = torch.from_numpy(acts.astype(np.float32))
    with torch.no_grad():
        return float(nn.functional.mse_loss(policy.net(X), y))
...
def run_flywheel(cfg: FlywheelConfig) -> dict:
    ...
    for si, name in enumerate(cfg.strategies):
        ...
        report: dict = {}
        s_kwargs = dict(cfg.strategy_kwargs.get(name, {}))

        for s in range(cfg.seeds):
            # stateful strategies (e.g. relabel_adaptive) get a fresh instance per seed
            strat = STRATEGY_MAKERS[name]() if name in STRATEGY_MAKERS else STRATEGIES[name]
            ...
            for it in range(1, cfg.iterations + 1):
                ...
                curated = strat(
                    rollouts, expert=oracle, dataset_states=db, dataset=dataset,
                    history=series, **s_kwargs,
                )
                if isinstance(curated, dict):
                    added = curated.get("add", [])
                    dataset.extend(added)
                    drop = curated.get("drop", [])
                    if drop:
                        drop_ids = {id(t) for t in drop}
                        dataset = [t for t in dataset if id(t) not in drop_ids]
                    cur_ratio = curated.get("ratio")
                else:
                    added = curated
                    dataset.extend(added)
                    cur_ratio = None
                ...
                if s == 0:
                    n_clean = sum(len(t) for t in dataset if t.source != "relabeled")
                    n_rel = sum(len(t) for t in dataset if t.source == "relabeled")
                    entry = {
                        "iteration": it,
                        "n_rollouts": len(rollouts),
                        "n_success": int(sum(r.success for r in rollouts)),
                        "n_curated": len(added),
                        "n_relabeled": int(sum(t.source == "relabeled" for t in added)),
                        "oracle_queries": int(sum(len(t) for t in added if t.source == "relabeled")),
                        "dataset_frames": sum(len(t) for t in dataset),
                        "n_clean_frames": n_clean,
                        "n_relabeled_frames": n_rel,
                        "mix_ratio": (n_rel / n_clean) if n_clean else float("inf"),
                        "rollout_success_rate": float(np.mean([r.success for r in rollouts])),
                    }
                    if cur_ratio is not None:
                        entry["controller_ratio"] = cur_ratio
                    if cfg.track_train_loss:
                        entry["train_mse"] = _bc_loss(policy, dataset, img_size=cfg.img_size)
                    curation_log.append(entry)
                    if cfg.verbose:
                        c = curation_log[-1]
                        print(
